...
def main():
    ...
    print("Save directory is:", os.path.abspath(save_dir))
    ...
    if soc_switch:
        ...
        for idx, data in enumerate(graph_dataset):
            ...
            orbital_energies, obrital_vecs = torch.linalg.eigh(Hs)
            ...
            eigen.append(orbital_energies.cpu().numpy())  # keep existing plotting logic

            
            eigen = np.swapaxes(np.array(eigen), 0, 1)*au2ev # (nbands, nk)
            eigvals_all = np.array(eigvals_all) * au2ev    # (Nk, n)
            eigvecs_all = np.array(eigvecs_all)            # (Nk, n, n)
            np.savez(
        ...
        for idx, data in enumerate(graph_dataset):
            ...
            # Convert to arrays and correct shapes
            eigen = np.array(eigen) * au2ev       # (nk, nbands)
            eigen = np.swapaxes(eigen, 0, 1)      # (nbands, nk)

            eigvals_all = np.array(eigvals_all) * au2ev  # (Nk, n)
            eigvecs_all = np.array(eigvecs_all)          # (Nk, n, n)

            # Save deliverable NPZ
            np.savez(
                os.path.join(save_dir, "bands_gmkg.npz"),
                kpoints=k_vec,
                eigenvalues=eigvals_all,
                eigenvectors=eigvecs_all,
                overlaps=np.array(overlaps_all)  # Add this line
            )
